...
function SessionRow({
  ...
  return (
    ...
    >
      {running
        ? <Loader2 size={12} className="mt-0.5 flex-shrink-0 animate-spin text-info-fg" />
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-muted-fg mt-0.5 opacity-70">
          {running ? <span className="text-info-fg animate-pulse">Running…</span> : timeAgo(session.updatedAt)}
  ...
  return (
    <>
      ...
      <div className="px-3 pb-2 border-b border-border">
        <div className="text-[10px] font-semibold text-muted-fg uppercase tracking-wider px-1 mb-1">
          Templates
        </div>
        {TEMPLATES.slice(0, expanded ? TEMPLATES.length : 3).map((t) => (
          ...
          >
            <span className="text-info-fg/70 flex-shrink-0">{t.icon}</span>
        ...
        <button
          ...
          className="w-full text-[11px] text-muted-fg hover:text-muted-fg py-1 transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1">
        ...
        {sessions.length === 0 ? (
          <p className="text-xs text-muted-fg px-3 py-3 text-center">No conversations yet</p>
        ) : (
