...
const BUILT_IN_SKILLS: SkillBundle[] = [
  {
    ...
    manifest: {
      name: 'dvalin-security-engineer',
      title: 'Dvalin Security Engineer',
      description: 'Orchestrate industry scanners, remediate confirmed findings, verify fixes, and prepare PR-ready security changes.',
      version: '1.0.0',
      builtIn: true,
      tools: ['run_security_suite', 'list_remediation_cases', 'prepare_remediation_worktree', 'run_check', 'git_diff'],
    },
    files: {
      [SKILL_MD]: [
        '# Dvalin Security Engineer',
        '',
        'Use this skill for end-to-end white-box security assessment and remediation.',
        '',
        'Workflow:',
        '1. Call `run_security_suite` and record which engines completed, were missing, or were policy-blocked.',
        '2. Treat scanner output as evidence to validate in source; prioritize critical/high findings and reachable data flows.',
        '3. Use an isolated remediation worktree for risky or multi-file fixes.',
        '4. Fix the vulnerability class without weakening tests, suppressing rules, or hiding findings.',
        '5. Run focused tests, relevant type/build checks, and `run_security_suite` again.',
        '6. Review `git_diff` and report fixed findings, verification evidence, remaining risk, and a draft-PR summary.',
        '7. Commit, push, or open a pull request only after the user explicitly requests publication.',
      ].join('\n'),
    },
  },
  {
